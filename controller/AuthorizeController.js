...
exports.authorizeUser = (request,response) => {
   ...
   response.redirect('https://accounts.spotify.com/authorize?'+querystring.stringify(credentials))
}
...
exports.generateToken = async (request,response) => {
   ...
   await fetch('https://accounts.spotify.com/api/token', {
      ...
      if (data.status == 200){
         ...
         console.log(d)
         d.expired_time = moment(moment().toDate().getTime()).add(1,'hours')
         response.cookie('spoitfyToken',d,{maxAge: d.expires_in * 1000 })     // milliseconds
         const spotifyData = new SpotifyModel({
            access_token:d.access_token,
            refresh_token:d.refresh_token,
            type:d.token_type,
            expires_in_seconds:d.expires_in,
            expires:d.expired_time
         })
         await spotifyData.save().then( (result,err) => {
            if (result){
